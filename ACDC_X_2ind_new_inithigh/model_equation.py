...
## equation for X only
##2 inducers
##Ara range optimised for cripsri data
## start with high red init state
...
parlist = [ 
    ...
    {'name' : 'K_b', 'lower_limit':-10.0,'upper_limit':-1.0}, 
...
ARA=np.array([0.0, 0.0000001,0.000003125,0.00000625,	0.0000125,	0.000025,	0.00005,0.0001,	0.0002,	0.2])

def Flow(X,Y,Z,ARA,par):
    #simplify version with less parameter    
    flow_x= 1 + (10**par['B_X']-1)*(np.power(ARA,par['n_a'])/( np.power(10**par['K_a'],par['n_a']) + np.power(ARA,par['n_a']))) 
    flow_x = flow_x / ( 1 + np.power((Z/(10**par['K_ZX'])),par['n_ZX']))
    flow_x = flow_x - X

    flow_y = 1 + (10**par['B_Y']-1)*( np.power(ARA,par['n_b'])) / ( np.power(10**par['K_b'],par['n_b']) + np.power(ARA,par['n_b']))
    flow_y = flow_y / ( 1 + np.power(X/(10**par['K_XY']),par['n_XY']))
    flow_y = flow_y - Y

    flow_z = 10**par['B_Z']/( 1 + np.power(Y/10**par['K_YZ'],par['n_YZ']))
...
def distance(x,pars,totaltime=tt, dt=dtt,trr=tr,N=node):
    ...
    for i in range(0,len(x)):
             ...
             if i==oscillation_ara[0] or i==oscillation_ara[1]: #1 concentration between not tested
                 d=0
            # print(d)
             d_final=d_final+d
...
def model(x,pars,totaltime=tt, dt=dtt,init=[3.,0,0]):
...
def solvedfunction2(Zi,ARA,par):
  ...
  #  Y=np.ones((len(ARA),len(Zi)))
   # Z=np.ones((len(ARA),len(Zi)))
    ...
    X= 1 + (10**par['B_X']-1)*(np.power(ARA,par['n_a'])/( np.power(10**par['K_a'],par['n_a']) + np.power(ARA,par['n_a']))) 
    X = X[:, None] / ( 1 + np.power((Zi/10**(par['K_ZX'])),par['n_ZX'])) 

    Y = 1 + (10**par['B_Y']-1)*( np.power(ARA,par['n_b'])) / ( np.power(10**par['K_b'],par['n_b']) + np.power(ARA,par['n_b']))
    Y = Y[:,None] / ( 1 + np.power(X/10**(par['K_XY']),par['n_XY']))

    Z = 10**par['B_Z']/( 1 + np.power(Y/10**(par['K_YZ']),par['n_YZ']))
...
def findss2(ARA,par):   
    ...
    for i,ai in enumerate(index[0]):
        ...
        X= 1 + (10**par['B_X']-1)*(np.power(ARA[ai],par['n_a'])/( np.power(10**par['K_a'],par['n_a']) + np.power(ARA[ai],par['n_a']))) 
        X = X / ( 1 + np.power((Z/10**(par['K_ZX'])),par['n_ZX']))
        Y = 1 + (10**par['B_Y']-1)*( np.power(ARA[ai],par['n_b'])) / ( np.power(10**par['K_b'],par['n_b']) + np.power(ARA[ai],par['n_b']))
        Y = Y / ( 1 + np.power(X/10**(par['K_XY']),par['n_XY']))
...
def jacobianMatrix(ARA,X,Y,Z,par):
    dxdx = -1
    dxdy = 0
    dxdz=-(((np.power(ARA,par['n_a'])*(10**par['B_X']-1))/ ( np.power(10**par['K_a'],par['n_a']) + np.power(ARA,par['n_a']))+1)*par['n_ZX']*np.power((Z/10**(par['K_ZX'])),par['n_ZX']))
    dxdz=dxdz/(Z*np.power((np.power((Z/10**(par['K_ZX'])),par['n_ZX'])+1),2))

    dydx=-(((np.power(ARA,par['n_b'])*(10**par['B_Y']-1))/ ( np.power(10**par['K_b'],par['n_b']) + np.power(ARA,par['n_b']))+1)*par['n_XY']*np.power((X/10**(par['K_XY'])),par['n_XY']))
    dydx=dydx/(X*np.power((np.power((X/10**par['K_XY']),par['n_XY'])+1),2))    
    dydy=-1
    dydz= 0

    dzdx = -(10**par['B_Z']*par['n_XZ']*np.power((X/10**par['K_XZ']),par['n_XZ']))
    dzdx= dzdx /((np.power((Y/10**par['K_YZ']),par['n_YZ'])+1)*X*np.power((np.power(X/10**par['K_XZ'],par['n_XZ'])+1) ,2))

    dzdy= -(10**par['B_Z']*par['n_YZ']*np.power((Y/10**par['K_YZ']),par['n_YZ']))
